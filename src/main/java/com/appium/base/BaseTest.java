/**
 * BaseTest is  class for common function like setup , teardown and we will extend all the test classes from this test base.
...
public class BaseTest {
	...
	public void beforeTest(@Optional("androidOnly") String emulator, String platformName, String udid,
		...
		ThreadContext.put(ROUTINGKEY, strFile); 		// Route logs to separate file for each thread
	...
	public void beforeSuite() throws AppiumServerHasNotBeenStartedLocallyException, Exception {
		...
		if (!AppiumServerUtils.checkIfAppiumServerIsRunning(APPIUM_PORT)) {
			server.start();
