...
import com.appium.enums.SeverityLevel;
...
public class LoginTests extends BaseTest {
	...
	@FrameworkAnnotation(severity = {SeverityLevel.Critical}, category = { CategoryType.SMOKE})
	@Test(groups = {"SMOKE" })
	public void invalidUserName() {
		JSONObject jsonObject_InvalidUser = 
				new JSONUtils().getJSONObject(TEST_DATA_JSON_FILE).getJSONObject(TEST_DATA_JSON_INVALID_USER);
		...
		String expectedErrTxt = StringsManager.getStrings().get(EXPECTED_DATA_KEY_ERR_INAVLID_CREDENTIALS);
		VerificationUtils.validate(actualErrTxt, expectedErrTxt, "Error Message for Invalid Credentials");
	}

	@FrameworkAnnotation(severity = {SeverityLevel.Major}, category = { CategoryType.REGRESSION})
	...
	public void invalidPassword() {
		...
		String username = jsonObject_InvalidUser.getString(TEST_DATA_JSON_USERNAME).toString();
		String password = jsonObject_InvalidUser.getString(TEST_DATA_JSON_PASSWORD).toString();
		loginPage.enterUsername(username).enterPassword(password).pressLoginBtn();
		String actualErrTxt = loginPage.getErrorTxt();
		String expectedErrTxt = StringsManager.getStrings().get(EXPECTED_DATA_KEY_ERR_INAVLID_CREDENTIALS);
		VerificationUtils.validate(actualErrTxt, expectedErrTxt, "Error Message for Invalid Credentials");
	}

	@FrameworkAnnotation(severity = {SeverityLevel.Major}, category = { CategoryType.SANITY, CategoryType.BVT, CategoryType.REGRESSION })
	...
	public void successfulLogin() {
		...
		productsPage = loginPage.enterUsername(username).enterPassword(password).pressLoginBtn();
